...
class PostToolUseHook(BaseHook):
    ...
    def process_hook(self, input_data: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Tool input keys: {list(tool_input.keys()) if isinstance(tool_input, dict) else 'Not a dict'}")
            ...
            logger.info(
                f"Tool success: {response_parsed['success']}, Result size: {response_parsed['result_size']} bytes")

            if response_parsed.get("error"):
                logger.warning(
                    f"Tool error detected: {response_parsed['error']}")
            ...
            logger.info(
                f"Database save result: {'Success' if save_success else 'Failed'}")
    ...
    try:
        ...
        try:
            ...
            if tool_response:
                ...
                logger.info(
                    f"Tool response size: {tool_response_size} characters")
