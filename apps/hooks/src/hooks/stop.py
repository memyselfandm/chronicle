...
class StopHook(BaseHook):
    ...
    def process_hook(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            event_data = {
                "event_type": "stop",
                     # Changed from "session_end" as per requirements
    ...
    def _count_session_events(self, session_id: str) -> int:
        ...
        try:
            ...
            if self.db_manager.supabase_client:
                try:
                    result = self.db_manager.supabase_client.table(self.db_manager.EVENTS_TABLE).select("id",
                        count="exact").eq("session_id", session_id).execute()
        ...
        try:
            ...
            if self.db_manager.supabase_client:
                try:
                    ...
                    self.db_manager.supabase_client.table(self.db_manager.SESSIONS_TABLE).update(update_data).eq("id",
                        session_id).execute()
                    return True
