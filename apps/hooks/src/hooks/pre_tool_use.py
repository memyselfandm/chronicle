...
class PreToolUseHook(BaseHook):
    ...
    def process_hook(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save event
            logger.info(
                f"Attempting to save pre_tool_use event for tool: {tool_name}")
            logger.info(
                f"Event data event_type: {event_data.get('event_type')}")
...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Tool input keys: {list(tool_input.keys()) if isinstance(tool_input, dict) else 'non-dict'}")

            # Log session ID extraction attempt
            session_id = input_data.get('session_id') or os.getenv('CLAUDE_SESSION_ID')
            logger.info(
                f"Session ID extracted: {'Yes' if session_id else 'No'}")
