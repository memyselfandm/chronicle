...
class ChronicleFilterDashboard {
    ...
    setupTimelineControls() {
        ...
        document.getElementById('timelineToggle').addEventListener('keydown', (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                this.toggleTimeline();
            }
    ...
    setupFilterToggle() {
        ...
        filterHeaderToggle.addEventListener('click', (e) => {
            // Prevent triggering preset dropdown if clicking on the preset button
            if (e.target.closest('.filter-presets-btn')) {
                return;
            }
            this.toggleFilterSection();
        });
        
        filterHeaderToggle.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                if (!e.target.closest('.filter-presets-btn')) {
                    this.toggleFilterSection();
    ...
    setupProjectGroupToggles() {
        document.querySelectorAll('.group-header').forEach(header => {
            header.addEventListener('click', (e) => {
                // Don't toggle if clicking on count or other interactive elements
                if (e.target.closest('.group-count')) {
                    return;
                }
    ...
    toggleTimeline() {
        ...
        // Update timeline toggle button text and aria
        const toggleBtn = document.getElementById('timelineToggle');
        const toggleText = toggleBtn.querySelector('span:not(.timeline-caret)');
        if (toggleText) {
            toggleText.textContent = isCollapsed ? 'Timeline' : 'Timeline';
        }
        
        // Store preference
        localStorage.setItem('chronicleTimelineCollapsed', !isCollapsed);
    }
    
    toggleProjectGroup(header) {
        const isCollapsed = header.classList.contains('collapsed');
        const projectId = header.dataset.project;
        const groupSessions = document.getElementById(`group-${projectId}`);
        
        header.classList.toggle('collapsed');
        header.setAttribute('aria-expanded', isCollapsed ? 'true' : 'false');
        
        // Store preference for this project
        const collapsedProjects = JSON.parse(localStorage.getItem('chronicleCollapsedProjects') || '[]');
        if (isCollapsed) {
        ...
        localStorage.setItem('chronicleCollapsedProjects', JSON.stringify(collapsedProjects));
    }
    ...
    toggleFilterSection() {
        ...
        headerToggle.setAttribute('aria-expanded', isCollapsed ? 'true' : 'false');
        
        // Store preference
        localStorage.setItem('chronicleFiltersCollapsed', !isCollapsed);
    }
    ...
    loadMockData() {
        ...
        const filtersCollapsed = localStorage.getItem('chronicleFiltersCollapsed') === 'true';
        const collapsedProjects = JSON.parse(localStorage.getItem('chronicleCollapsedProjects') || '[]');
        ...
        if (filtersCollapsed) {
            ...
            document.getElementById('filterHeaderToggle').setAttribute('aria-expanded', 'false');
        }
        
        // Restore collapsed project states
        collapsedProjects.forEach(projectId => {
            const header = document.querySelector(`[data-project="${projectId}"]`);
            if (header) {
    ...
    addMockEvent() {
        const mockEvents = [
            ...
            { type: 'notification', session: 'sess-1', details: 'Awaiting user confirmation for changes' },
            { type: 'error', tool: 'Bash', session: 'sess-2', details: 'Command failed with exit code 1' },
            { type: 'stop', session: 'sess-4', details: 'Session ended by user' },
        ];
