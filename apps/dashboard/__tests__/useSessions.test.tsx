...
describe('useSessions Hook', () => {
  ...
  describe('Session End Time Updates', () => {
    it('should update session end times from stop events', async () => {
      const mockSessions = [
        {
          id: 'session-1',
      ...
      // Mock stop event query
      const mockStopEvents = [
        {
      ...
      mockFrom.eq.mockReturnValueOnce(mockFrom); // For session events filter
      mockFrom.in.mockReturnValueOnce(mockFrom); // For event type filter
      mockFrom.order.mockReturnValueOnce(mockFrom); // For ordering
      mockFrom.limit.mockResolvedValueOnce({ data: mockStopEvents, error: null });
      ...
      await act(async () => {
        await result.current.updateSessionEndTimes();
      });

      expect(mockFrom.update).toHaveBeenCalledWith({ 
        end_time: '2024-01-01T01:00:00Z' 
      });
      expect(mockFrom.eq).toHaveBeenCalledWith('id', 'session-1');
    });
    ...
    it('should handle update errors gracefully', async () => {
      ...
      // Mock stop event exists
      mockFrom.eq.mockReturnValueOnce(mockFrom);
      mockFrom.in.mockReturnValueOnce(mockFrom);
      mockFrom.order.mockReturnValueOnce(mockFrom);
      mockFrom.limit.mockResolvedValueOnce({ 
        data: [{ timestamp: '2024-01-01T01:00:00Z', event_type: 'stop' }], 
        error: null 
      });
    ...
    it('should skip sessions that already have end times', async () => {
      const mockSessions = [
        {
          id: 'session-1',
          start_time: '2024-01-01T00:00:00Z',
          end_time: '2024-01-01T01:00:00Z', // Already has end time
        },
      ];
      ...
      // Should not query for stop events for sessions with end times
      expect(mockFrom.eq).not.toHaveBeenCalledWith('session_id', 'session-1');
    });
  });
